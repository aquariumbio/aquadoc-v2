  ...
  class Render
    ...
    def sample_type_link(name)
      if @options[:inventory] && name
        "<a href='#' onclick='easy_select(\"Sample Types\", \"#{name}\")'>" \
        "#{name}</a>"
    ...
    def object_type_link(name)
      if @options[:inventory] && name
        "<a href='#' onclick='easy_select(\"Containers\", \"#{name}\")'>" \
        "#{name}</a>"
      elsif !@options[:inventory] && name
        name
      else
    ...
    def op_type_md(operation_type_spec)
      ...
      inputs = ot[:field_types].select do |ft|
        ft[:role] == 'input' && ft[:ftype] == 'sample'
      end
      params = ot[:field_types].select do |ft|
        ft[:role] == 'input' && ft[:ftype] != 'sample'
      end
      outputs = ot[:field_types].select do |ft|
        ft[:role] == 'output' && ft[:ftype] == 'sample'
      end
